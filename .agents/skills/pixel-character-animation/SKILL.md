...
4. 生成後檢查輪廓、角色身份、透明背景、像素邊緣、比例與幀間一致性；不合格的創意或畫面內容只針對問題素材重新生成。若只是不透明背景、畫布尺寸、透明 alpha 或置中等可明確判定的格式問題，才可用確定性的資產整理流程處理，不重畫角色像素。遇到角色大小跳動時，必須先分離「角色本體」與「武器／尾巴／觸手等外伸裝備」的量測，不可直接用包含長武器的完整外框當本體比例。
...
- 角色本身的大小以頭部、軀幹、四肢等穩定身體核心為準；長槍、盾牌、尾巴、觸手或特效的外伸距離不能反過來決定身體縮放。
- 要統一一個方向的動畫時，先對該方向的 idle／move／attack 幀建立共同的身體核心聯合外框，再用同一比例縮放該方向的完整圖片。這樣武器仍可保留在畫布內，但不會讓某個動作或朝上方向把角色放大／縮小。
- 身體核心量測可使用 `alpha >= 64` 的遮罩、細長外伸部位排除或連通元件等確定性方法；這些方法只用於量測與格式整理，不可藉此重畫或任意縮短角色裝備。
- 若武器在視覺上確實凸出過多，應把「縮短／改畫武器」視為獨立的美術內容修正；不能用縮小整個角色來掩蓋它。
...
- 每一幀維持相同角色身份、基準位置、畫布尺寸、比例、材質與主色；避免尺寸跳動、姿勢漂移、臉部變形、道具消失或跨幀重疊。尺寸 QA 應比較身體核心，不要把武器外框的變化誤判成角色本體大小變化。
...
- 若每張生成圖的透明留白或外框不同，先依需求選擇基準：需要整體可見範圍一致時，才逐張擷取完整非透明區域；需要角色本體一致時，改用身體核心聯合外框，並保留完整來源畫布讓武器不參與本體縮放。兩者都是顯示格式標準化，不是重畫角色內容。
- 選角、玩家列表與對局應共用同一個角色繪製入口；其他角色與怪物維持原本路徑，除非需求明確擴大範圍。
- 只有在使用者要求「套用／整合／修改程式」時才修改程式；單純預覽或生成圖片要求不修改遊戲程式碼。

## 可重複驗收

- 資產：數量、命名、方向、幀數、畫布尺寸、四角透明、未貼邊、無透明 RGB 污染、角色本體核心比例、角色中心與武器完整性。
- 顯示：固定顯示尺寸、最近鄰像素邊緣、八方向（包含上方與斜上方的正確背面）、idle／move／attack 優先序、跨幀本體大小一致、沒有常駐底圈或意外外框。
