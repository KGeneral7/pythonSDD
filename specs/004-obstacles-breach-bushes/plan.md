...
> **版本範圍**：本計畫記錄 v0.2.0 的矩形地形實作與驗收。文中 18／27／45 個物件、162/162 測試及 172.23 FPS 均為 v0.2.0 歷史數據；目前工作樹已由 `specs/007-map-asset-integration/` 改用 100×100px 獨立地形格，最新結果以 007 文件為準。
...
| VII. 依 Spec Kit 管理分支與 PR 生命週期 | 功能目錄、文件與本次交付分支使用 004-obstacles-breach-bushes；003 的散彈修正列為相容性補充，外部生命週期由 PR #4、v0.2.0 與後續文件同步 PR 收尾 | 交付、文件同步與本次相關已合併遠端分支清理已完成；本地 squash 後非 ancestry 的交付與文件分支經一般 `git branch -d` 檢查遭拒，依規範未使用 `-D`，故保留並記錄 |

### 設計門檻結論

設計門檻通過。T002 已確認既有工作內容保留，T031～T035 的本地驗證已完成；本次以 `codex/004-obstacles-breach-bushes` 作為交付分支，PR #4 已完成合併並發布 v0.2.0，後續文件同步 PR 亦已合併。本次相關且已合併的遠端開發分支均已刪除；本地 squash 後提交不在 `main` ancestry 的交付與文件分支，經一般 `git branch -d` 安全檢查遭拒，依規範保留，不修改或重置 `day3/` 等未納入檔案。
...
初始工作樹基線曾受未提交的 GUI、自動瞄準、投射物與戰鬥變更影響；本輪保留既有使用者內容，並讓地形無關的既有技能 fixture 使用空障礙物清單以維持測試責任邊界。v0.2.0 整合完成時，完整套件為 162/162 通過；沒有以回復或重置使用者變更的方式消除基線差異。
...
- v0.2.0 驗證時完整測試為 162/162 通過；若後續其他工作樹再次出現 GUI 或既有功能基線差異，應獨立記錄，不覆蓋未提交 `config.py`。
