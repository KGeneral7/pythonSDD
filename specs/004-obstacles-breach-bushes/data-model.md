...
> **版本範圍**：本資料模型記錄 v0.2.0 的矩形地形狀態與驗收數據。文中 18／27／45 個物件及 162/162 測試均為歷史數據；目前工作樹的正式地圖已由 `specs/007-map-asset-integration/` 改為 100×100px 獨立格。
...
- v0.2.0 的 `update_world` 已接入移動/技能地形解析與恢復順序；完整自動化測試為 162/162 通過。
