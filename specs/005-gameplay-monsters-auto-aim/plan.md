...
**原始交付分支**：`codex/003-combat-vfx-cone-ammo`。本功能與 003、004 共用 `world.py`、`rendering.py` 與測試整合，已隨 [PR #3](https://github.com/KGeneral7/pythonSDD/pull/3) 與 `v0.1.0` 完成；後續的 004 地圖更新由 `codex/004-obstacles-breach-bushes` 另行交付。
...
6. 執行 `python -m unittest pvpve_escape.tests.test_game_features`、完整 `pvpve_escape/tests` 回歸、`python -m compileall -q pvpve_escape` 與 `git diff --check`；本功能當時的完整套件基線為 162/162 通過，目前工作樹的最新整合結果以 007 功能驗收文件為準。
