...
| `viewport_visible` | 布林值 | `surface.get_rect().collidepoint(screen_position)`；只作為其他玩家的畫面視野剔除條件 | 其他玩家錨點在 viewport 外時不繪製公開頭頂資訊；本機玩家不套用此剔除 |
...
| 查看對象 | 顯示內容 | 明確隱藏內容 |
|---|---|---|
| 本機玩家（`player.player_id == viewer_id`；目前 `viewer_id=0`） | 名稱/編號、生命條/數值、彈藥、配件圓圈、大招百分比、強化層數；死亡時另顯示畫面中央倒數 | 無；不套用其他玩家的 viewport 剔除 |
| 位於地形可見範圍且錨點在 viewport 內的其他存活玩家 | 名稱/編號、生命條/數值 | 彈藥、配件顏色、大招百分比、強化層數、死亡倒數 |
| 位於地形可見範圍且錨點在 viewport 內的其他死亡玩家 | 名稱/編號、空生命條/0 生命 | 同上；不得因死亡而顯示私有資源 |
| 錨點在 viewport 外的其他玩家 | 不繪製頭頂公開資訊；保留角色圖形的既有 viewport 裁切，回到 viewport 後恢復 overlay | 不得因 overlay 邊界夾取而在畫面邊緣殘留 |
...
| 移動或鏡頭追蹤 | `position` 或 `camera.position` 改變 | 所有可見覆蓋層重新由世界座標投影，保持在對應玩家上方；其他玩家錨點離開 viewport 時停止繪製，回來後恢復 |
...
- 其他玩家錨點完全落在 viewport 外時，不得使用上述邊界夾取繼續繪製其血量條或其他頭頂資訊；只有錨點回到 viewport 內才可恢復。
