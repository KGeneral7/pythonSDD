...
- 程式改由 `config.MAX_FPS = 120` 統一正式遊戲與地圖編輯器的畫面更新上限，並保留 `FPS` 相容別名。
- 完整 unittest：229 項通過；`compileall` 與 `git diff --check`：通過。
- 暖機 120 幀後量測 600 次更新／繪製，約 8.844 秒、67.84 FPS；600 是量測次數，不是執行期上限。
- [PR #15](https://github.com/KGeneral7/pythonSDD/pull/15) 已合併，並發布 [v0.5.1](https://github.com/KGeneral7/pythonSDD/releases/tag/v0.5.1)。
